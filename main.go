...
func main() {
   ...
   router.GET("/alive", keephimalive)
   router.GET("/trace", oauthTraceG)
   router.POST("/trace", oauthTraceP)
   router.GET("/login/oauth/authorize", oauthAuthorize)
   //router.GET("/verify-auth/", oauthVerifyAuth)
   ...
   fmt.Println("Listening to 0.0.0.0:80")
...
///////////////// API
func oauthTraceP(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
   log.Println("oauthTrace POST function called")
   v := req.URL.Query()
   log.Println(v.Encode())
}
func oauthTraceG(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
   log.Println("oauthTrace GET function called")
   v := req.URL.Query()
   log.Println(v.Encode())
}
///////////////// API
//func oauthVerifyAuth(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
// log.Println("oauthVerifyAuth function called")
// v := req.URL.Query()
// target := rancher_url + "/verify-auth?" + v.Encode()
// log.Println(target)
// log.Println(v.Encode())
// http.Redirect(w, req, target, http.StatusTemporaryRedirect)
//}
func oauthAuthorize(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
   log.Println("oauthAuthorize function called")

   v := req.URL.Query()
   v.Add("response_type", "code")
   v.Set("scope", "read_api")
   //v.Set("redirect_uri", sso_url + "/verify-auth/")
...
func oauthAccessToken(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
   log.Println("oauthAccessToken function called")
   v := req.URL.Query()
...
func apiV3User(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
   log.Println("apiV3User function called")
...
func apiV3UserId(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
   log.Println("apiV3UserId function called")
...
func keephimalive(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
   fmt.Fprintf(w, "i am alive")
   log.Println("keephimalive function called")
}

func apiV3UserOrgs(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
   log.Println("apiV3UserOrgs function called")
...
func apiV3UserTeams(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
   log.Println("apiV3UserTeams function called")
   gitlabClient := createGitlabClient(req)
   allAvailable := false
...
func apiV3TeamsId(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
   ...
   log.Println("apiV3TeamsId function called")
   gitlabClient := createGitlabClient(req)
...
func apiV3SearchUsers(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
   log.Println("apiV3SearchUsers function called")
...
func createGitlabClient(req *http.Request) *gitlab.Client {
   log.Println("createGitlabClient function called")
...
func convertGitlabUserToAccount(gitlabUser *gitlab.User) *Account {
   log.Println("convertGitlabUser function called")
   return &Account{
      ID:        gitlabUser.ID,
...
func convertGitlabGroupToAccount(gitlabGroup *gitlab.Group) *Account {
   log.Println("convertGitlabGroupToAccount function called")
   return &Account{
      ID:        gitlabGroup.ID,
...
func convertGitlabGroupToTeam(gitlabGroup *gitlab.Group) *Team {
   log.Println("convertGitlabGroupToTeam function called")
